...
const ForecastPage = () => {
    ...
    const [days, setDays] = useState(forecast?.days as number)

    const [calcfield, setCalcfield] = useState(forecast?.price as number)
    ...
    useEffect(() => {
        if (forecast) {
            setDays(forecast?.days as number)
            setCalcfield(forecast?.price as number)
        }
    }, [forecast]);
    ...
    return (
        <Form onSubmit={sendForecast} className="pb-5">
            ...
            <Row className="mb-5 fs-5 w-25">
                <CustomInputNumber label="Кол-во дней аренды" placeholder="Введите кол-во дней" value={days || 0} setValue={setDays} disabled={!isDraft}/>
                {calcfield ?
                    <CustomInputNumber label="Стоимость" value={calcfield} disabled={true}/> :
                    <CustomInputText label="Стоимость" value="Не расчитано" disabled={true}/>
                }
            </Row>
